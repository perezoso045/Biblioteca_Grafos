...
class Grafo:
    ...
    def dibujar(self,WHITE,RED,BLUE,screen,clock,nombre):
            pygame.display.set_caption(nombre)  # Título de la ventana
            for llave, nodo in self.nodos.items():
            ...
            clock.tick(60)  # Establecer el límite de la tasa de cuadros por segundo

    def Spring(self,WHITE,RED,BLUE,screen,clock,k_repulsion, k_atraccion, iteraciones, k, max_velocidad, umbral_distancia):
        running = True
        for _ in range(iteraciones):  # Realizar las iteracciones especificadas
                ...
                for llave2, nodo2 in self.nodos.items():
                    if nodo1 != nodo2:  # Evitar la fuerza entre un nodo y él mismo
                        ...
                        if distancia > umbral_distancia:
                            fuerza_repulsion = k_repulsion / distancia**2  # Ley de Coulomb
            ...
            self.dibujar(WHITE,RED,BLUE,screen,clock,"Spring")  # Llamar a la función de dibujo para mostrar el grafo
    ...
    # Método para aplicar el algoritmo de Fruchterman-Reingold
    def Fruchterman(self, WHITE, RED, BLUE, screen, clock, k_repulsion, k_atraccion, iteraciones, k, umbral_distancia):
        for llave, nodo in self.nodos.items():
            nodo.coordenadas=[random.randint(500,1200), random.randint(300, 700)]
        # Bucle principal que realiza iteraciones sobre el algoritmo
        running = True
        for iteration in range(iteraciones):  # Ejecuta por el número de iteraciones especificado
            for llave, nodo in self.nodos.items():
                nodo.fuerzas[0] = 0.0  # Inicializar la fuerza en X para el nodo
                nodo.fuerzas[1] = 0.0  # Inicializar la fuerza en Y para el nodo
            ...
            for llave1, nodo1 in self.nodos.items():
                for llave2, nodo2 in self.nodos.items():
                    if nodo1 != nodo2:
                        ...
                        distancia = math.sqrt((nodo1.coordenadas[0] - nodo2.coordenadas[0]) ** 2 + (nodo1.coordenadas[1] - nodo2.coordenadas[1]) ** 2) or 1
                        if distancia < umbral_distancia:  # Solo aplicar repulsión si la distancia es menor que el umbral
                            # Calcular la fuerza de repulsión basada en la distancia
                            force = k_repulsion**2 / distancia
                            difx = (nodo1.coordenadas[0] - nodo2.coordenadas[0]) * force
                            dify = (nodo1.coordenadas[1] - nodo2.coordenadas[1]) * force
                            nodo1.fuerzas[0] += difx  # Actualizar la fuerza en X
                            nodo1.fuerzas[1] += dify  # Actualizar la fuerza en Y
                            nodo2.fuerzas[0] -= difx  # Actualizar la fuerza en X
                            nodo2.fuerzas[1] -= dify  # Actualizar la fuerza en Y
    
            # Calcular fuerzas de atracción según las aristas del grafo
            for llave, arista in self.aristas.items():
                # Calcular la distancia entre los dos nodos de la arista
                distancia = math.sqrt((arista.n1.coordenadas[0] - arista.n2.coordenadas[0]) ** 2 + (arista.n1.coordenadas[1] - arista.n2.coordenadas[1]) ** 2) or 1
                if distancia < umbral_distancia:  # Solo aplicar atracción si la distancia es menor que el umbral
                    force = distancia**2 / k_atraccion  # Calcular la fuerza de atracción
                    difx = (arista.n2.coordenadas[0] - arista.n1.coordenadas[0]) * force
                    dify = (arista.n2.coordenadas[1] - arista.n1.coordenadas[1]) * force
                    # Actualizar las fuerzas de los nodos de la arista
                    arista.n1.fuerzas[0] -= difx
                    arista.n1.fuerzas[1] -= dify
                    arista.n2.fuerzas[0] += difx
                    arista.n2.fuerzas[1] += dify
    
            # Actualizar las posiciones de los nodos basadas en las fuerzas calculadas
            for llave, nodo in self.nodos.items():
                # Limitar la velocidad para evitar que los nodos se muevan demasiado rápido
                max_velocidad = math.sqrt(nodo.fuerzas[0]**2 + nodo.fuerzas[1]**2) or 1
                nodo.fuerza[0] = (nodo.fuerzas[0] / max_velocidad)*k
                nodo.fuerza[1] = (nodo.fuerzas[1] / max_velocidad)*k
                nodo.coordenadas[0] += nodo.fuerza[0]  # Actualizar la posición en X
                nodo.coordenadas[1] += nodo.fuerza[1]  # Actualizar la posición en Y
    
            # Dibujar el grafo actualizado
            screen.fill(WHITE)  # Llenar el fondo con color blanco
            self.dibujar(WHITE, RED, BLUE, screen, clock, "Fruchterman")
    ...
    # Método para aplicar el algoritmo de Barnes-Hut, optimizando el cálculo de repulsión
    def Barnes_hut(self, WHITE, RED, BLUE, screen, clock, k_repulsion, k_atraccion, iteraciones, k,n, umbral_distancia):
        running = True
        for _ in range(iteraciones):  # Realizar las iteraciones especificadas
            for llave, nodo in self.nodos.items():
                nodo.fuerzas[0] = 0.0  # Inicializar las fuerzas en X
                nodo.fuerzas[1] = 0.0  # Inicializar las fuerzas en Y

            # Crear un quadtree para optimizar la búsqueda de puntos cercanos
            quadtree = Quadtree((0, 0, 1100, 700), n)
            for llave, nodo in self.nodos.items():
                quadtree.insert(tuple(nodo.coordenadas))  # Insertar las coordenadas de los nodos en el quadtree
            screen.fill(WHITE)  # Llenar el fondo con color blanco
            quadtree.dibujar(screen, (0, 255, 0))  # Color verde para los cuadrantes
            for llave1, nodo1 in self.nodos.items():
                # Consultar el quadtree para obtener puntos cercanos para la repulsión
                region = (0, 0, 1100, 700)
                puntos_cercanos = quadtree.query(region)

                for punto in puntos_cercanos:
                    difx = nodo1.coordenadas[0] - punto[0]
                    dify = nodo1.coordenadas[1] - punto[1]
                    distancia = math.sqrt(difx ** 2 + dify ** 2) or 1

                    # Solo aplicar la repulsión si la distancia supera el umbral
                    if distancia > umbral_distancia:
                        # Calcular la fuerza de repulsión usando la distancia
                        fuerza_repulsion = k_repulsion / distancia**2
                        nodo1.fuerzas[0] += fuerza_repulsion * difx / distancia
                        nodo1.fuerzas[1] += fuerza_repulsion * dify / distancia

            # Fuerzas de atracción entre los nodos conectados por aristas
            for llave, arista in self.aristas.items():
                difx = arista.n1.coordenadas[0] - arista.n2.coordenadas[0]
                dify = arista.n1.coordenadas[1] - arista.n2.coordenadas[1]
                distancia = math.sqrt(difx ** 2 + dify ** 2) or 1
                if distancia > umbral_distancia:
                    # Calcular la fuerza de atracción basada en la distancia
                    fuerza_atraccion = -k_atraccion * math.log(distancia)
                    arista.n1.fuerzas[0] += fuerza_atraccion * difx / distancia
                    arista.n1.fuerzas[1] += fuerza_atraccion * dify / distancia
                    arista.n2.fuerzas[0] -= fuerza_atraccion * difx / distancia
                    arista.n2.fuerzas[1] -= fuerza_atraccion * dify / distancia

            # Actualizar las posiciones de los nodos en base a las fuerzas calculadas
            for llave, nodo in self.nodos.items():
                nodo.coordenadas[0] += nodo.fuerzas[0] * k  # Actualizar posición en X
                nodo.coordenadas[1] += nodo.fuerzas[1] * k  # Actualizar posición en Y

            # Dibujar el grafo actualizado
            self.dibujar(WHITE, RED, BLUE, screen, clock, "Barnes-Hut")
