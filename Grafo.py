...
class Grafo:
    ...
    def Calcular_fuerzas(self, k_repulsion, k_atraccion, iteracciones, k, max_velocidad, umbral_distancia):
        ...
        pygame.display.set_caption("Dibujo de un grafo")

        def dibujar():
            screen.fill(WHITE)
            for llave, nodo in self.nodos.items():
                pygame.draw.circle(screen, RED, nodo.coordenadas, 8)
            # Dibujar aristas (conexiones entre los nodos)
            for llave, arista in self.aristas.items():
                pygame.draw.line(screen, BLUE, arista.n1.coordenadas, arista.n2.coordenadas, 1)
        ...
        for _ in range(iteracciones):
            ...
            for llave1, nodo1 in self.nodos.items():
                ...
                for llave2, nodo2 in self.nodos.items():
                    if nodo1 != nodo2:
                        difx = nodo1.coordenadas[0] - nodo2.coordenadas[0]
                        dify = nodo1.coordenadas[1] - nodo2.coordenadas[1]
            ...
            for llave, nodo in self.nodos.items():
                ...
                nodo.coordenadas[0] += nodo.fuerza[0]
                nodo.coordenadas[1] += nodo.fuerza[1]
