...
def Malla(m, n):
    ...
    obj = Grafo('Archivos_GV/Malla de ' + str(m) + ' por ' + str(n))  # Inicializa un grafo con el nombre correspondiente
    for i in range(m * n):
        obj.agregar_nodo(i)  # Agrega nodos al grafo numerados del 1 al m*n
    matriz = [[(i * m + j ) for j in range(m)] for i in range(n)]  # Crea una matriz para representar la malla
    for i in range(n):
        for j in range(m):
            ...
            for di, dj in movimientos:
                ...
                if (0 <= ni < n) and (0 <= nj < m):  # Verifica si el vecino está dentro de los límites
                    if not obj.repetida(num, matriz[ni][nj]):  # Verifica que la arista no sea repetida
...
def ErdosRenyi(n, a):
    ...
    obj = Grafo('Archivos_GV/Erdos-Renyi de ' + str(n) + ' nodos y ' + str(a) + ' aristas')  # Inicializa el grafo
    for i in range(n):
...
def Gilbert(n, p):
    ...
    obj = Grafo('Archivos_GV/Gilbert de ' + str(n) + ' nodos y probabilidad de conexion de ' + str(p))  # Inicializa el grafo
    for i in range(n):
        obj.agregar_nodo(i)  # Agrega nodos al grafo
    for n1 in obj.nodos:
        for n2 in obj.nodos:
...
def Geografico(n, r):
    ...
    obj = Grafo('Archivos_GV/Geografico de ' + str(n) + ' nodos y distancia ' + str(r))  # Inicializa el grafo
...
def BarabasiAlbert(n, d):
    ...
    obj = Grafo('Archivos_GV/BarabasiAlbert de ' + str(n) + ' nodos de grado ' + str(d))  # Inicializa el grafo
    for i in range(n):
        obj.agregar_nodo(i)  # Agrega nodos al grafo
    for i in range(d):
        for j in range(d):
...
def DorogovtsevMendes(n):
    ...
    obj = Grafo('Archivos_GV/DorogovtsevMendes de ' + str(n) + ' nodos')  # Inicializa el grafo
    for i in range(n):
        obj.agregar_nodo(i)  # Agrega nodos al grafo
